...
const HIGHLIGHT_METADATA_RE =
  /^(?:原文链接|链接|hn讨论|讨论链接|讨论|发布日期|发布时间|pdf下载|官网|数据来源|来源|分数|评论|hn排名|排名|作者|时间|投票|话题|状态|product hunt|website|link|discussion|release date|published|pdf download|source|score|comments?|hn rank|rank|author|time|votes?|topics?|status|state)\s*[:：]/i;
...
function isPureLinkLine(raw: string): boolean {
  const normalized = stripLeadingPictographs(
    raw
      .trim()
      .replace(/^\*\*(.+)\*\*$/, "$1")
      .trim(),
