...
const HIGHLIGHT_STATUS_RE =
  /(?:^|[:：—-]\s*)(?:no new (?:content|data|releases?(?: today)?|versions?|updates?)|no releases? in the last 24 hours|nothing new|no activity(?: in the last 24 hours)?|no updates?|no notable activity|skipping report|暂无(?:内容|活动|更新|新版本)?|无新(?:内容|数据|版本(?:发布)?|更新|活动)|无(?:数据|活动|更新|新增)|未发现(?:新内容|更新)?|跳过|过去24小时无(?:活动|新版本发布))[.!。]?$/i;

const HIGHLIGHT_FAILURE_STATUS_RE =
  /^(?:(?:report generation|generation|llm call|fetch|request)\s+(?:failed|failure)|failed to (?:generate|fetch|load|read|build)|(?:报告生成|生成|模型调用|llm调用|抓取|请求)(?:失败|错误)|失败)(?:\s*[:：].*)?[.!。]?$/i;
