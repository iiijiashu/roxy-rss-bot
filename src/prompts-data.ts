...
const HIGHLIGHT_STATUS_RE =
  /^(?:(?:none[.:]?\s*)?no new (?:content|data|versions?|updates?|releases?(?: today)?|releases?(?: were)? (?:published|released)(?: today| in the last 24 hours)?)|no releases? in the last 24 hours|nothing new|no activity(?: in the last 24 hours)?|no updates?|no notable activity|skipping report|(?:今日|今天)?\s*(?:暂无|无)\s*新(?:的)?\s*(?:内容|数据|版本(?:发布)?|更新|活动|pr\s*更新|pr)|暂无(?:内容|活动|更新|新版本)|无(?:数据|活动|更新|新增)|未发现(?:新内容|更新)?|跳过|过去24小时无(?:活动|新版本发布))[.!。]?$/i;
...
const HIGHLIGHT_METADATA_RE =
  /^(?:原文链接|链接|hn讨论|讨论链接|讨论|发布日期|发布时间|pdf下载|官网|数据来源|来源|分数|评论|hn排名|排名|作者|时间|投票|话题|product hunt|website|link|discussion|release date|published|pdf download|source|score|comments?|hn rank|rank|author|time|votes?|topics?)\s*[:：]/i;
...
function isHighlightMetadata(raw: string): boolean {
  const cleaned = cleanHighlightText(raw);
  return HIGHLIGHT_METADATA_RE.test(cleaned) || /^https?:\/\/\S+$/i.test(cleaned);
}
