...
const HIGHLIGHT_STATUS_RE =
  /^(?:(?:none[.:]?\s*)?no new (?:content|data|versions?|updates?|releases?(?: today)?|releases?(?: were)? (?:published|released)(?: today| in the last 24 hours)?)|no new content detected(?:,?\s*skipping report)?|no releases? in the last 24 hours|nothing new|no activity(?: in the last 24 hours)?|no updates?|no notable activity|skipping report|\(?[^()]*no content to analyze[.!]?\)?|(?:今日|今天)?\s*(?:暂无|无)\s*新(?:的)?\s*(?:内容|数据|版本(?:发布)?|更新|活动|pr\s*更新|pr)|暂无(?:内容|活动|更新|新版本)|无(?:数据|活动|更新|新增)|未发现(?:新内容|更新)?|跳过|过去24小时无(?:活动|新版本发布)|（?[^（）]*暂无可供分析的内容[。.]?）?)[.!。]?$/i;
...
const HIGHLIGHT_METADATA_RE =
  /^(?:原文链接|链接|hn讨论|讨论链接|讨论|发布日期|发布时间|pdf下载|官网|数据来源|来源|分数|评论|hn排名|排名|作者|时间|日期|投票|话题|状态|product hunt|website|link|discussion|release date|published|pdf download|source|score|comments?|hn rank|rank|author|time|date|crawl period|coverage period|votes?|topics?|status|state)\s*[:：]/i;
...
const HIGHLIGHT_SUBJECT_HEADER_RE =
  /^(?:项目|标题|论文|模型|产品|文章|故事|名称|仓库|工具|project|title|paper|model|product|article|story|name|repository|repo|tool)$/i;
const HIGHLIGHT_DETAIL_HEADER_RE =
  /^(?:简要说明|一句话(?:说明|解读)|摘要|说明|解读|核心观点|亮点|summary|description|insight|takeaway)$/i;
