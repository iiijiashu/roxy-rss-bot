...
const HIGHLIGHT_STATUS_RE =
  /^(?:(?:none[.:]?\s*)?no new (?:content|data|versions?|updates?|releases?(?: today)?|releases?(?: were)? (?:published|released)(?: today| in the last 24 hours)?)|no new content detected(?:,?\s*skipping report)?|no releases? in the last 24 hours|nothing new|no activity(?: in the last 24 hours)?|no updates?|no notable activity|skipping report|\(?[^()]*no content to analyze[.!]?\)?|(?:今日|今天)?\s*(?:暂无|无)\s*新(?:的)?\s*(?:内容|数据|版本(?:发布)?|更新|活动|pr\s*更新|pr)|暂无(?:内容|活动|更新|新版本)|无(?:数据|活动|更新|新增)|未发现(?:新内容|更新)?|跳过|过去\s*24\s*小时\s*内?\s*无\s*(?:活动|新版本发布)|（?[^（）]*暂无可供分析的内容[。.]?）?)[.!。]?$/i;

const HIGHLIGHT_FAILURE_STATUS_RE =
  /^(?:(?:skills\s+)?summary generation failed|trending report generation failed|(?:report generation|generation|llm call|fetch|request)\s+(?:failed|failure)|failed to (?:generate|fetch|load|read|build)|(?:skills\s*)?摘要生成失败|趋势报告生成失败|(?:报告生成|生成|模型调用|llm调用|抓取|请求)(?:失败|错误)|失败)(?:\s*[:：].*)?[.!。]?$/i;

const HIGHLIGHT_METADATA_RE =
  /^(?:原文链接|链接|hn讨论|讨论链接|讨论|发布日期|发布时间|pdf下载|官网|数据来源|来源|分数|评论|hn排名|排名|作者|时间|日期|投票|话题|状态|product hunt|website|link|discussion|release date|published|pdf download|source|score|comments?|hn rank|rank|author|time|date|crawl date|crawl period|coverage period|focus period|votes?|topics?|status|state)\s*[:：]/i;

const AUTO_GENERATED_FOOTER_RE = /^(?:本日报由|this digest is auto-generated by)\s+agents-radar\b/i;
...
function cleanHighlightText(raw: string): string {
  return raw
    ...
    .replace(/\s+/g, " ")
    .trim()
    .replace(/^(?:一句话(?:说明|解读)|简介|简要说明)\s*[:：]\s*/u, "");
}

function isHighlightNoise(raw: string): boolean {
  const cleaned = cleanHighlightText(raw).replace(
    /^(?:(?:(?:\p{Extended_Pictographic}|\p{Emoji_Presentation})\uFE0F?(?:\u200D(?:\p{Extended_Pictographic}|\p{Emoji_Presentation})\uFE0F?)*)\s*)+/u,
    "",
  );
  return HIGHLIGHT_STATUS_RE.test(cleaned) || HIGHLIGHT_FAILURE_STATUS_RE.test(cleaned);
}

function isHighlightMetadata(raw: string): boolean {
  const cleaned = stripLeadingPictographs(cleanHighlightText(raw));
  return (
    HIGHLIGHT_METADATA_RE.test(cleaned) ||
    /^https?:\/\/\S+$/i.test(cleaned) ||
    /^(?:anthropic|openai)\s*[:：]\s*(?:(?:https?:\/\/)?(?:www\.)?(?:anthropic|openai)\.com\b)/i.test(cleaned)
  );
}

function stripLeadingPictographs(raw: string): string {
  return raw
    .replace(
      /^(?:(?:(?:\p{Extended_Pictographic}|\p{Emoji_Presentation})\uFE0F?(?:\u200D(?:\p{Extended_Pictographic}|\p{Emoji_Presentation})\uFE0F?)*)\s*)+/u,
      "",
...
function isAutoGeneratedFooter(raw: string): boolean {
  return AUTO_GENERATED_FOOTER_RE.test(cleanHighlightText(raw));
}

const HIGHLIGHT_GRAPHEME_SEGMENTER = new Intl.Segmenter(undefined, { granularity: "grapheme" });

function highlightGraphemes(text: string): string[] {
  return Array.from(HIGHLIGHT_GRAPHEME_SEGMENTER.segment(text), ({ segment }) => segment);
}

function truncateHighlight(text: string, maxChars: number): string {
  const chars = highlightGraphemes(text);
...
function truncateHighlightWithTail(text: string, maxChars: number): string {
  const chars = highlightGraphemes(text);
  if (chars.length <= maxChars) return text;
  const suffixLength = Math.min(6, Math.max(2, Math.floor(maxChars / 5)));
  const prefixLength = Math.max(1, maxChars - suffixLength - 1);
  return `${chars.slice(0, prefixLength).join("")}…${chars.slice(-suffixLength).join("")}`;
}

function highlightKey(text: string): string {
  return cleanHighlightText(text)
    .normalize("NFKC")
    .toLowerCase()
    .replace(/[^\p{L}\p{N}]+/gu, "");
}
...
function markdownTableCells(line: string): string[] {
  ...
  for (let index = 0; index < body.length; index++) {
    ...
    if (char === "`") {
      ...
      current += body.slice(index, index + runLength);
      index += runLength - 1;
      continue;
    }
...
interface HighlightTableSchema {
  subjectIndex: number;
  detailIndex: number;
}

const HIGHLIGHT_SUBJECT_HEADER_RE =
  /^(?:项目|标题|论文|模型|产品|文章|故事|名称|仓库|工具|功能方向|project|title|paper|model|product|article|story|name|repository|repo|tool|issue(?:\s*\/\s*pr)?|pr|bug|feature|skill)$/i;
const HIGHLIGHT_DETAIL_HEADER_RE =
  /^(?:简要说明|一句话(?:说明|解读)|摘要|说明|解读|核心观点|核心价值|阅读理由|关键动态|具体诉求|技术路线|亮点|summary|description|insight|takeaway|impact|why it matters(?: today)?)$/i;

function highlightTableSchema(headerLine: string): HighlightTableSchema | null {
  const headers = markdownTableCells(headerLine).map((cell) => cleanHighlightText(cell));
  // Relevance-screening tables are process metadata, not final recommendations.
  // Skipping the whole table prevents both retained and explicitly excluded
  // repositories from consuming Telegram highlight slots.
  if (headers.some((header) => /^(?:判定|decision|relevance)$/i.test(header))) return null;
  const subjectIndex = headers.findIndex((header) => HIGHLIGHT_SUBJECT_HEADER_RE.test(header));
  const detailIndex = headers.findIndex((header) => HIGHLIGHT_DETAIL_HEADER_RE.test(header));
  if (subjectIndex < 0 || detailIndex < 0 || subjectIndex === detailIndex) return null;
  return { subjectIndex, detailIndex };
}

function isPureLinkLine(raw: string): boolean {
  const normalized = stripLeadingPictographs(raw.trim().replace(/\*\*/g, "").trim());
  if (/^https?:\/\/\S+$/i.test(normalized)) return true;
  const parts = normalized.split(/\s*\|\s*/u).map((part) => part.trim());
  return (
    parts.length > 0 &&
    parts.every((part) => /^https?:\/\/\S+$/i.test(part) || /^\[[^\]]+\]\([^)]+\)$/.test(part))
  );
}

function tableHighlight(line: string, schema: HighlightTableSchema): string | null {
  const cells = markdownTableCells(line).map((cell) => cleanHighlightText(cell));
  const subject = cells[schema.subjectIndex]?.trim() ?? "";
  const detail = cells[schema.detailIndex]?.trim() ?? "";
  if (!subject || !detail) return null;
  if (isHighlightNoise(detail) || isHighlightMetadata(detail) || isAutoGeneratedFooter(detail)) return null;
  return `${subject}：${detail}`;
}

function markdownPrefixColumns(prefix: string): number {
  let columns = 0;
...
function highlightCandidates(content: string): string[] {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (fence && line && indent < fenceIndent) fence = null;
    // Nested/shorter markers and info text cannot close a fence.
    if (fence) {
      const closing = line.match(/^(`{3,}|~{3,})\s*$/)?.[1];
      if (closing && closing[0] === fence[0] && closing.length >= fence.length) fence = null;
      continue;
    }
    const list = rawLine.match(/^([ \t]*(?:[-*+]|\d+[.)])[ \t]+)(.*)$/);
    const opening = (list?.[2] ?? line).match(/^(`{3,}|~{3,})(.*)$/);
    const marker = opening?.[1];
    // Backtick fence info cannot itself contain backticks (e.g. an inline span).
    if (marker && (marker[0] !== "`" || !opening?.[2]?.includes("`"))) {
      fence = marker;
      fenceIndent = markdownPrefixColumns(list?.[1] ?? "");
      tableSchema = null;
      continue;
    }
    if (inSummary) {
    ...
    if (line.startsWith("|")) {
      ...
      if (isMarkdownTableSeparator(nextLine)) {
        ...
        // as `Metric | Claude Code | Codex` intentionally produce no candidates.
        tableSchema = highlightTableSchema(line);
        continue;
      }
      if (!tableSchema) continue;
      const table = tableHighlight(line, tableSchema);
    ...
    if (bullet) {
      if (isPureLinkLine(bullet)) continue;
      const cleaned = cleanHighlightText(bullet);
      if (
        cleaned &&
        !isHighlightMetadata(cleaned) &&
        !isHighlightNoise(cleaned) &&
        !isAutoGeneratedFooter(cleaned)
      ) {
    ...
    if (isPureLinkLine(line)) continue;
    const paragraph = cleanHighlightText(line);
    if (
      paragraph.length >= 12 &&
      !isHighlightMetadata(paragraph) &&
      !isHighlightNoise(paragraph) &&
      !isAutoGeneratedFooter(paragraph)
    ) {
...
/**
 * Build notification highlights directly from finished reports.
 *
 * Daily highlights are a presentation layer over reports that have already
 * been generated by the LLM. Keeping this extraction deterministic avoids
 * spending another provider request (and another failure point) merely to
 * re-summarize content that already exists on disk.
 */
export function extractReportHighlights(
  ...
  for (const [reportId, content] of Object.entries(reportContents)) {
    ...
    const items: string[] = [];
    for (const candidate of highlightCandidates(content)) {
      const fullText = cleanHighlightText(candidate);
      const fullKey = highlightKey(fullText);
      if (!fullText || !fullKey || seenFull.has(fullKey)) continue;
      if (lang === "zh" && !/[\u3400-\u9fff]/u.test(fullText)) continue;

      seenFull.add(fullKey);
      let rendered = truncateHighlight(fullText, maxChars);
      let renderedKey = highlightKey(rendered);
      if (seenRendered.has(renderedKey)) {
        rendered = truncateHighlightWithTail(fullText, maxChars);
        renderedKey = highlightKey(rendered);
      }
      if (seenRendered.has(renderedKey)) {
        const marker = `·${items.length + 1}`;
        const markerChars = highlightGraphemes(marker);
        const renderedChars = highlightGraphemes(rendered);
        rendered = `${renderedChars.slice(0, Math.max(1, maxChars - markerChars.length)).join("")}${marker}`;
        renderedKey = highlightKey(rendered);
      }
    ...
    if (items.length > 0) result[reportId] = items;
  }
